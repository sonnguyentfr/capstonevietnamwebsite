...
namespace NVCMS.API.ReadGoogleSheet.Jobs
{
    ...
    public class CampaignBatchJob
    {
        ...
        public async Task ExecuteAsync(
            int campaignId, string subject, string body, int emailAccountId,
            CancellationToken cancellationToken = default)
        {
            _logger.LogInformation(
                "CampaignBatchJob: start campaignId={CampaignId}", campaignId);

            var queuedLogs = (await _sendLogRepo.GetQueuedByCampaignIdAsync(campaignId)).ToList();
            _logger.LogInformation(
                "CampaignBatchJob: {Count} queued send-logs", queuedLogs.Count);
            ...
            for (int i = 0; i < queuedLogs.Count; i += BatchSize)
            {
                ...
                foreach (var log in batch)
                {
                    try
                    {
                        var sesMessageId = await _sesService.SendBodyEmailAsync(
                            log.Email, string.Empty, subject, body);

                        await _sendLogRepo.UpdateStatusAsync(
                            log.Id, MailSendStatus.Sent, sesMessageId: sesMessageId);
                    ...
                    {
                        await _sendLogRepo.UpdateStatusAsync(
                            log.Id, MailSendStatus.Failed, errorMessage: ex.Message);

                        totalFailed++;
                        _logger.LogError(ex, "Failed to send to {Email} logId={LogId}",
                            log.Email, log.Id);
                    }
                }
            ...
            _logger.LogInformation(
                "CampaignBatchJob: done campaignId={CampaignId} sent={Sent} failed={Failed}",
