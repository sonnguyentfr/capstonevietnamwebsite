...
#Region "Event Handlers"
        ...
        Public Function GetTopMenu() As String
            ...
            'Lấy cache
            Dim cache As Hashtable = DataCache.GetCache(Of Hashtable)("MenusTopMenuCacheSetting" + sLanguage)
            ...
            If Not cache.ContainsKey("MenusTopMenuCacheSetting" + sLanguage) Then
                If File.Exists(PortalSettings.HomeDirectoryMapPath & "\MenusXML\" & "MainMenu.xml") Then
                    ...
                    cache.Item("MenusTopMenuCacheSetting" + sLanguage + ModuleId.ToString) = xmlDoc
                    ...
                    For Each objMenusInfo As XmlNode In arrMainCateInfo
                        ...
                        If objMenusInfo.ChildNodes.Count = 0 Then
                            sbMenu.AppendFormat("<li class='{0}'><a href=""{1}"" target={2}>{3}</a></li>", MenuActive(iTabid), link, iTaget, name)
                        Else
                            sbMenu.AppendFormat("<li class='{0} dropdown'>", MenuActive(iTabid))
                            sbMenu.AppendFormat("<a href='{0}' class='dropdown-toggle'>{1}</a>", link, name)
                ...
                If (DotNetNuke.Common.Globals.PerformanceSettings.HeavyCaching <> PerformanceSettings.NoCaching) Then
                    DataCache.SetCache("MenusTopMenuCacheSetting" + sLanguage, cache)
                End If
            End If
