...
namespace DesktopModules.TinTuc.ViewPage
{
    public partial class Index : PortalModuleBase
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    if (Request.QueryString["cateId"] == null)
                    {
                        ...
                        NV_NewsCategoriesInfo category = cateController.GetByID(CategoryID);
						if (category != null)
                        {
                    ...
                    {
                        CategoryID = Request.QueryString["cateId"] != null ? Convert.ToInt32(Request.QueryString["cateId"]) : 143;
                        NV_NewsCategoriesController cateController = new NV_NewsCategoriesController();
                        NV_NewsCategoriesInfo category = cateController.GetByID(CategoryID);
						if (category != null)
                        {
                        ...
                        tp.Title = category.CategoryName;
                    }
        ...
        {
            NV_NewsController controller = new NV_NewsController();
            string sessionSubHotCat = "";
            if (Session["SubtractIdsHotCat"] != null)
            {
                sessionSubHotCat = Session["SubtractIdsHotCat"].ToString();
            }
            TotalRecord = controller.SelectCount(sessionSubHotCat, CategoryID, 0, "", false);
            ...
            if (!cache.ContainsKey(cachestring))
            {
                //Lay thong tin cat
                string sTemplate = "";
                string sTemplateFile = Server.MapPath("/Portals/0/NewsTemplates/") + setting_template;
                ...
                string caturl = DotNetNuke.Common.Globals.NavigateURL(category.TabID);
                sTemplate = sTemplate.Replace(TOKEN_CATNAME, category.CategoryName).Replace(TOKEN_CATURL, caturl);

                //Lay tin nong
                ArrayList listHott = new ArrayList();
                if (setting_hot > 0)
                {
                    listHott = controller.ShowSelectTopCatNewsHOT("", CategoryID, setting_hot, PortalId);
                    if (listHott.Count == 0)
                    {
                    ...
                    {
                        string sTemplate_list = sTemplate.Contains(TOKEN_HOT_ITEM) ? TrimToken(sTemplate, TOKEN_HOT_ITEM) : "";
                        string sListHot = "";
                        for (int i = 0; i < listHott.Count; i++)
                        {

                            NV_NewsInfo news = (NV_NewsInfo)listHott[i];
                            SubtractIds += news.NewId + ",";
                            sListHot += ToHTML(sTemplate_list, news, (i + 1), setting_imgWidthHOT, setting_imgHeightHOT);
                        }
                        sTemplate = sTemplate.Replace(sTemplate_list, sListHot).Replace("[" + TOKEN_HOT_ITEM + "]", "").Replace("[/" + TOKEN_HOT_ITEM + "]", "");

                    }
                }
                ArrayList list = new ArrayList();
                string cachename = BL.NewsCatList + CategoryID + CurrentPage + ModuleId;
                if (setting_order == ORDER_DATE_DESC)
                    list = controller.SelectIndex(sessionSubHotCat, CategoryID, 0, CurrentPage, PageSize, "", false);
                else
                    list = controller.SelectIndex(sessionSubHotCat, CategoryID, 0, CurrentPage, PageSize, "", false);
                ...
                else
                {
                    ...
                    {
                        NV_NewsInfo news = (NV_NewsInfo)list[i];
        ...
        private string ToHTML(string sTemplate, NV_NewsInfo news, int position, int imgWidth, int imgHeight)
        {
            try
            {
                int tabID = BL.GetMappingTabIDByCategoryID(news.CategoryId);
                ...
                string newid = Convert.ToString(news.NewId);
                string date = news.PublishedDate.ToShortDateString();
                string thu = news.PublishedDate.ToString("dd");
                string nam = news.PublishedDate.ToString("yyyy");
                string reglink = news.SourceText;
                string ngay = news.PublishedDate.ToString("MM", new CultureInfo("vi-VN"));
                string expiredDate = ""; // news.ExpiredDate.ToShortDateString();
                string image = "";
                string datecount = Ultis.ToRelativeDate(news.PublishedDate);
                string attachFile = news.AttachedFiles != "" ? news.AttachedFiles : "#";
                ...
                if (news.SourceText != "")
                {
                    Uri myUri = new Uri(news.SourceText);
        ...
        private bool LoadSetting()
        {
            ...
            {
                //tin Nong
                if (!Null.IsNull(ModuleConfiguration.ModuleSettings[BL.settingListHot_PageSize.ToString()]))
                {
                    setting_hot = Convert.ToInt32(ModuleConfiguration.ModuleSettings[BL.settingListHot_PageSize.ToString()].ToString());
                }
                else setting_hot = 0;
                //Image size
