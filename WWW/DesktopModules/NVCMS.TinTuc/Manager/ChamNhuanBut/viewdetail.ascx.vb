...
Namespace DesktopModules.TinTuc.Manager

    Public MustInherit Class adminnews_inc_list
...
#Region "Event Handlers"
        ...
        Private Sub BinddrgDataViewer()
            Try
                ...
                TotalRecord = ctlnhuabut.NhuanBut_Find_Count(CType(Datefrom, Date), CType(DateTo, Date), uid, 0, 0, 0, 0)
                ...
                If (arr.Count > 0) Then
                    For i As Integer = 0 To arr.Count - 1
                        ...
                        If obj.Membership.Approved = True Then
                            ...
                            'isobaiviet = ctl.AdminFindStatus_Index(CType(Datefrom, Date), CType(DateTo, Date), KeySearch, 0, PortalId, NewsStatus.DaXuatBan, obj.UserID, "vi-VN", CurrentPage, PageSize)
                            isobaiviet = ctlnhuabut.NhuanBut_Find_Count(CType(Datefrom, Date), CType(DateTo, Date), obj.UserID, 0, 0, 0, 0)
                            If (isobaiviet > 0) Then
                                strtable += "<tr><td class='thangtacgia' style='padding: 5px; background: #efefef;'>" & obj.DisplayName & "</td><td>" & isobaiviet & "</td><td>" & Counttype(obj.UserID, 0) & "</td><td>" & Counttype(obj.UserID, 7) & "</td><td>" & Counttype(obj.UserID, 2) & "</td><td>" & Counttype(obj.UserID, 3) & "</td><td>" & Counttype(obj.UserID, 4) & "</td><td>" & Counttype(obj.UserID, 5) & "</td><td>" & Counttype(obj.UserID, 6) & "</td><td>" & Counttype(obj.UserID, 9) & " </td><td>" & Counttype(obj.UserID, 12) & "</td></tr>"
                            End If
                        End If
                    Next
                End If
        ...
        Function Counttype(Userids As Integer, types As Integer) As Integer
            ...
            icount = ctlnhuabut.NhuanBut_Find_Count(CType(Datefrom, Date), CType(DateTo, Date), Userids, 0, types, PortalId, 0)
            Return icount
        End Function
        ...
        Private Sub lbtFind_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lbtFind.Click
            Try
                ...
                Response.Redirect(NavigateURL(TabId) & "?view=display" & "&uid=" & ddlUserPost.SelectedValue & "&from=" & Me.txtStartdate.Value & "&to=" & Me.txtEndDate.Value)
            Catch ex As Exception
                ProcessModuleLoadException(Me, ex)
            End Try
        End Sub
