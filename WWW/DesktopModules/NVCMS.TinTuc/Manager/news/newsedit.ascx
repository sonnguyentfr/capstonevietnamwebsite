<%@ Control Language="vb" AutoEventWireup="false" Explicit="true" CodeFile="newsedit.ascx.vb" Inherits="DesktopModules.TinTuc.Manager.news.newsedit" EnableViewState="true" %>
...
<link rel="stylesheet" href="/static/_admin/assets/css/libs/jstree.css?ver=2.7.1">
                            ...
                            <div class="col-md-12 col-lg-12 col-xxl-12">
                                <div class="card card-bordered h-100">
                                    <div class="card-inner">
                                        ...
                                        <div class="form-group">
                                            <div class="form-control-wrap">
                                                <asp:TextBox ID="txtmeta_title" Font-Names="Nunito" runat="server" CssClass="form-control  form-control-outlined editor-font"></asp:TextBox>
                                                <label class="form-label-outlined" for="<%=txtmeta_title.ClientID %>">Meta Title</label>
                                                <div id="seometatitle" class="chuanseo col-sm-12"></div>
                                                <mark>Nếu để trống hệ thống sẽ lấy Tiêu đề, Mô tả làm chuẩn</mark>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <div class="form-control-wrap">
                                                <asp:TextBox ID="txtmeta_url" Font-Names="Nunito" runat="server" CssClass="form-control  form-control-outlined editor-font"></asp:TextBox>
                                                <label class="form-label-outlined" for="<%=txtmeta_url.ClientID %>">Slug</label>
                                                <mark>Nếu bài viết đã xuất bản thì không sửa được Slug</mark>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <div class="form-control-wrap">
                                                <asp:TextBox ID="txtmeta_description" runat="server" CssClass="form-control form-control-outlined editor-font" Height="60px" TextMode="MultiLine" MaxLength="160"></asp:TextBox>
                                                <label class="form-label-outlined" for="<%=txtmeta_description.ClientID %>">Meta Description</label>
                                                <div id="seometadesc" class="chuanseo col-sm-12"></div>
                                                <mark>Nếu để trống hệ thống sẽ lấy Tiêu đề, Mô tả làm chuẩn</mark>
                                            </div>
                                        </div>
                                                                ...
                                                                <ItemTemplate>
                                                                    <li class="anh-daupload">
                                                                        ...
                                                                        <div class="anh-thongtin">
                                                                            <asp:Button ID="btnxoaanh" Visible='<%#ChoXoaAnh(Eval("mediaid")) %>' CommandArgument='<%#Eval("mediaid") %>' CommandName="btnxoaanh" Text="Xóa" OnClick="btnxoaanh" CssClass="anh-addToContent btn" runat="server" title="Xóa file khỏi dữ liệu" />
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#<%=txtmeta_title.ClientID%>").on('keyup paste', function () {
            var Characters = $("#<%=txtmeta_title.ClientID%>").val().replace(/(<([^>]+)>)/ig, "").length;
            var remaining = 70 - Characters;

            $("#seometatitle").text(remaining);

            if (remaining < 0) {
                $("#seometatitle").css("color", "red");
            } else {
                $("#seometatitle").css("color", "");
            }
        });
        $("#<%=txtmeta_description.ClientID%>").on('keyup paste', function () {
            var Characters = $("#<%=txtmeta_description.ClientID%>").val().replace(/(<([^>]+)>)/ig, "").length;
            var remaining = 300 - Characters;

            $("#seometadesc").text(remaining);

            if (remaining < 0) {
                $("#seometadesc").css("color", "red");
            } else {
                $("#seometadesc").css("color", "");
            }
        });
        $("#<%=txtmeta_url.ClientID%>").on('blur', function () {
            var str = $(this).val();

            var slug = removeVietnameseTones(str);

            // thay space thành -
            slug = slug.replace(/\s+/g, '-');

            // bỏ các ký tự không phải chữ, số, dấu -
            slug = slug.replace(/[^a-zA-Z0-9\-]/g, '');

            // chuyển thành lowercase
            slug = slug.toLowerCase();

            // bỏ các dấu - thừa ở đầu hoặc cuối
            slug = slug.replace(/^-+|-+$/g, '');
...
<script language="javascript" type="text/javascript">
    ...
    function FetchData() {
        ...
        metatitle = Base64.encode($('#<%=txtmeta_title.ClientID %>').val());
        metadesc = Base64.encode($('#<%=txtmeta_description.ClientID %>').val());
        metaslug = Base64.encode($('#<%=txtmeta_url.ClientID %>').val());
