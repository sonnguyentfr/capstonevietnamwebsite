...
#Region "Event Handlers"
        Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            Try
                ...
                If activetab = BL.pageTheoDoiTinBaiId Then
                    PageSuaBai = BL.pageSuaTheoDoiTinBai
                ...
                If Not IsPostBack Then
                    ...
                    If IsNumeric(Request.Item("status")) Then
                        Me.Status = Request.Item("status")
                        ddlStatus.SelectedValue = Status
                    End If
                    If Not String.IsNullOrEmpty(Request.Item("isactive")) Then
                        chkIsActive.Checked = Request.Item("isactive")
                        lbtShowNews.Visible = chkIsActive.Checked
                        lbtDelete.Visible = chkIsActive.Checked
                        lbtHideNews.Visible = Not chkIsActive.Checked
                    End If
        ...
        Private Sub BinddrgDataViewer()
            Try
                Dim isActive As Boolean = Not chkIsActive.Checked
                Dim ctl As New NV_NewsController
                TotalRecord = ctl.AdminFind_Count(Datefrom, DateTo, KeySearch, CategoryId, PortalId, Status, CreatedUser, "", isActive, False)
                ...
                Dim arr As ArrayList = ctl.AdminFind_Index(Datefrom, DateTo, KeySearch, CategoryId, PortalId, Status, CreatedUser, "", isActive, False, CurrentPage, PageSize)
                'For Each obj As NV_NewsInfo In arr
                '    If obj.Unit = BL.GetPhongBanIdByUserId(PortalId, UserId) Then
                '        arrChinhThong.Add(obj)
                '    Else
                '        arrNhoXL.Add(obj)
                '    End If
                'Next
                'arrResult.AddRange(arrChinhThong)
                'arrResult.AddRange(arrNhoXL)
        ...
        Private Sub BindDdlStatus()
            ...
            Me.ddlStatus.Items.Insert(0, New ListItem("-- Chọn trạng thái --", "-1"))
        End Sub
        ...
        End Sub
        Private Sub lbtFind_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lbtFind.Click, ddlPageSize.SelectedIndexChanged, ddlCategory.SelectedIndexChanged, ddlUserPost.SelectedIndexChanged, ddlStatus.SelectedIndexChanged, chkIsActive.CheckedChanged
            Try
                Response.Redirect(NavigateURL(TabId) & "?key=" & txtTitle.Text & "&catid=" & ddlCategory.SelectedValue & "&uid=" & ddlUserPost.SelectedValue & "&status=" & ddlStatus.SelectedValue & "&isactive=" & chkIsActive.Checked & "&from=" & Me.txtStartdate.Text & "&to=" & Me.txtEndDate.Text & "&pageNo=" & ddlPageSize.SelectedValue)
            Catch ex As Exception
                ProcessModuleLoadException(Me, ex)
            End Try
        End Sub
        ...
        ''' <param name="e"></param>
        Protected Sub lbtHideNews_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbtHideNews.Click
            Dim i As Integer
            ...
            For i = 0 To (Me.drgDataViewer.Items.Count - 1)
                Try
                    chkBox = CType(Me.drgDataViewer.Items(i).FindControl("chkRow"), CheckBox)
                    If Not chkBox Is Nothing Then
                        If chkBox.Checked = True Then
                            Dim newsid As Integer = CType(Me.drgDataViewer.Items(i).FindControl("lblnewid"), Label).Text
                            ctlNews.UpdateVisible(newsid, False)
                            'Version
                            With objProcessInfo
                                .NewsID = newsid
                                .CreateDate = DateTime.Now
                                .ByUser = UserId
                                .StatusID = ctlNews.GetByID(newsid).Status
                ...
                End Try
            Next
            ClientAPI.RegisterStartUpScript(Me.Page, "UpdateSuccess", "<script>UpdateSuccess('Xóa tin bài thành công!');</script>")
            BinddrgDataViewer()
        End Sub
        ''' <summary>
        ''' Xóa bay khỏi CSLD
            ...
            For i = 0 To (Me.drgDataViewer.Items.Count - 1)
                Try
                    ...
                    If Not chkBox Is Nothing Then
                        If chkBox.Checked = True Then
                            Dim newsid As Integer = CType(Me.drgDataViewer.Items(i).FindControl("lblnewid"), Label).Text
                            Ultis.NewsHardDelete(Server, newsid)
                        End If
                    End If
                Catch ex As Exception
                    ProcessModuleLoadException(Me, ex)
