...
#End Region
        ...
        Private Sub BinddrgDataViewer()
            Try
                ...
                TotalRecord = ctl._Find_Count("", KeySearch, Status, PortalId)
                ...
                drgDataViewer.DataSource = ctl._Find_Index("", KeySearch, Status, PortalId, CurrentPage, PageSize)
        ...
        Private Sub BindddlStatus()
            ddlStatus.Items.Add(New ListItem("Đang biên soạn", "1"))
            ddlStatus.Items.Add(New ListItem("Chưa xuất bản", "2"))
            ddlStatus.Items.Add(New ListItem("Đã xuất bản", "3"))
            Me.ddlStatus.DataBind()
            Me.ddlStatus.Items.Insert(0, New ListItem(" - Tất cả - ", "0"))
        End Sub
        ...
        Protected Sub cmdquickview(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            Dim ctlNews As New Question_Controller
            Dim objhistory As Question_Info = ctlNews._GetByID(itemidhistory, PortalId)
            If Not objhistory Is Nothing Then
                With objhistory
                    Me.lblhAuthor.Text = BL.GetButDanh(PortalId, .UserId)
                    ltlngaydang.Text = .CreatedDate.ToString("HH:mm dd/MM/yyy")
                    ltrcauhoi.Text = .CauHoi
